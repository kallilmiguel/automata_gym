...
for i in last_action:
    for j in range(cases):
            fsInt.append((final_states_int.count((i,j+1)), 10*(j+1), env.mapping()[i][1], "int"))
            fsRdn.append((final_states_rdn.count((i,j+1)), 10*(j+1), env.mapping()[i][1], "rnd"))



listfsI=states_int.values.tolist()
list2Int=[]
for i in range(0,9):
    list2Int.append((listfsI[i][2],listfsI[i][1], listfsI[i+9][1],listfsI[i+18][1],listfsI[i+27][1]))

listfsR=states_rdn.values.tolist()
list2Rdn=[]
for i in range(0,9):
    list2Rdn.append((listfsR[i][2],listfsR[i][1], listfsR[i+9][1],listfsR[i+18][1],listfsR[i+27][1]))

fsInt = pd.DataFrame(list2Int,columns=["fp","Rejection Type 1","Rejection Type 2", "Approval Type 1","Approval Type 2"])
fsRdn = pd.DataFrame(list2Rdn,index="fp", columns=["fp","Rejection Type 1","Rejection Type 2", "Approval Type 1","Approval Type 2"])

fsInt.to_csv("final_states_nsarsa_int.csv")
fsRdn.to_csv("final_states_nsarsa_random.csv")

data = np.vstack((info_int, info_rdn))
data = pd.DataFrame(data, columns=["mean reward", "Fail Probability", "method"])
data.to_csv("data_nsarsa.csv")
df = pd.read_csv("data_nsarsa.csv")
...
states_int = pd.read_csv("final_states_nsarsa_int.csv")
states_rdn = pd.read_csv("final_states_nsarsa_random.csv")

states_int = states_int.drop(["Unnamed: 0"], axis=1)
states_rdn = states_rdn.drop(["Unnamed: 0"], axis=1)

plt = states_int.plot.bar(x="fp",stacked=True)
plt.set_xlabel("Fail Probability")
plt.set_ylabel("Number of Occurrences")

plt = states_rdn.plot.bar(x="fp",stacked=True)
plt.set_xlabel("Fail Probability")
plt.set_ylabel("Number of Occurrences")
...
states_int = pd.read_csv("final_states_sarsa_int.csv")
states_rdn = pd.read_csv("final_states_sarsa_random.csv")
